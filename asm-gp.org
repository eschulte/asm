             ...
             (select-asm
              (concat
               (dorun
                (pmap #(evaluate-asm %)
                      (concat
                       (take (* crossover-rate population-size)
                             (repeatedly
                              (apply crossover-asm (select-asm population 2))))
                       (pmap #(mutate-asm %)
                             (select-asm population
                                         (* (1 - crossover-rate population-size)))))))
               population)
              population-size)
             (+ generation 1)))))))
#+end_src
...
#+begin_src clojure :tangle scripts/modify :shebang #! /usr/bin/env clj
  ...
  (cmd/with-command-line (rest *command-line-args*)
    ...
    (def tournament-size tour-size)
    (def max-section-size sect-size)
    (def crossover-rate cross-rate)
