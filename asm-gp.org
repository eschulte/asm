...
* COMMENT Tasks [27/30]
** TODO smaller disk footprint
- delete binary file after evaluation
- delete .S file after compilation
- modify read-obj and write-obj to use compression
      ...
      (assoc asm
        :fitness (if bin
                   (try
                    ...
                    (.delete (java.io.File.bin))
                    (if (.exists (java.io.File. out-file))
                      (count (f/read-lines out-file))
                      0)
                    (catch Exception e (do (.delete (java.io.File.bin)) 0)))
                   0))))
...
- the high level experiment runner script
  #+begin_src sh :tangle ../neutral/runner.sh :shebang #!/bin/bash
    ...
    #
    for i in `seq $2`;do
        echo "nice neutral --out-num $3 $1.`expr $i - 1`.clj > $1.$i.clj"
        nice neutral --out-num $3 $1.`expr $i - 1`.clj > $1.$i.clj
        # bail if for some reason we didn't get enough
        [[ $3 -ne `cat $1.$i.clj|grep -i ":fitness 10"| \
