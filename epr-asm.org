...
[[file:code.org][source code]] and [[file:instructions.org][instructions for reproduction]] are provided.  The git
repository of this project is available with
: git clone http://gitweb.adaptive.cs.unm.edu/asm.git
...
1) The source-code and all support files are available at
   [[file:asm-gp.tar.bz2][asm-gp.tar.bz2]], and through git at
   http://gitweb.adaptive.cs.unm.edu/asm.git.
...
* COMMENT Tasks [28/30]
** DONE smaller disk footprint [3/3]
- [X] delete binary file after evaluation
- [X] delete .S file after compilation
- [X] modify read-obj and write-obj to use compression -- this has now
      been done with cread-obj and cwrite-obj

NOTE: this has only fully been applied to the neutral space stuff.

** DONE exploration of neutral space
   :PROPERTIES:
   :CUSTOM_ID: exploration-of-neutral-space
   :END:
1) start with a set of working programs (maybe only one) which could
   be generated from a =.S= file with something like
   #+begin_src clojure
     (cwrite-obj "results/pop.0.clj.gz" (list (read-asm "base.s")))
   #+end_src
...
- the related =test.sh= script
  #+begin_src sh :tangle ../neutral/test.sh :shebang #!/bin/bash
    ...
    exit 0
  #+end_src
- resource limiting script for safer evaluation -- =limit10.c=
  enhanced with some restrictive calls to =setrlimit=
  #+begin_src c :tangle scripts/limit.c
    #include <stdio.h>
    #include <unistd.h> 
    #include <sys/time.h>
  ...
  #+end_src
- the high level experiment runner script
  #+begin_src sh :tangle ../neutral/runner.sh :shebang #!/bin/bash
    #
    # take a series of $2 steps in $3 walks in neutral space
    #
    for i in `seq $2`;do
        echo "nice neutral --out-num $3 $1.`expr $i - 1`.clj.gz > $1.$i.clj.gz"
        nice neutral --out-num $3 $1.`expr $i - 1`.clj.gz |gzip > $1.$i.clj.gz
        rm /tmp/variant-*.*
        # bail if for some reason we didn't get enough
        [[ $3 -ne `cat $1.$i.clj.gz |gunzip |grep -i ":fitness 10"| \
            wc|awk '{print $1}'` ]] && break
    done
  #+end_src
